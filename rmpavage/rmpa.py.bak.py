...
class RoadmakersPavage:
    ...
    def create_feat_table(self,ignore_padding=True):
        ...
        for node in set(self.WG.nodes()):
            ...
            if ignore_padding:
                diffs = [self.WG.node[node]['value']-self.WG.node[n]['value'] for n in self.WG.neighbors(node) if n!=0]
            else:
            ...
            if 0 in diffs:#if it is not unique then cluster
                ...
                self.feat_table[node] = {'scale':self.WG.node[node]['scale'],'value':self.WG.node[node]['value'],'isMax':True} 
            elif all(item < 0 for item in diffs):#is local minumum
                self.feat_table[node] = {'scale':self.WG.node[node]['scale'],'value':self.WG.node[node]['value'],'isMax':False}
#        node = 0
        elapsed_time = time.process_time() - t
...
#        diffs = [self.WG.node[node]['value']-self.WG.node[n]['value'] for n in self.WG.neighbors(node)]
        ...
        if not diffs:#if there are no neighbors not equal to the zeroth node:
            if (len(self.WG) ==1):#if this is the zeroth node:
                ...
                self.PG.add_node(0,self.WG.node[node]['scale'])#value, scale 
                self.PG.add_edges_from([(i,len(self.PG)) for i in self.WG.node[node]['pulses']])
                self.WG.remove_node(node)
            else:
                nearest_node = 0
                self.PG.add_node(self.WG.node[node]['value'],self.WG.node[node]['scale']) # Add a new node to PG with (value j,  scale equal to node's scale)           
                self.PG.add_edges_from([(i,len(self.PG)) for i in self.WG.node[node]['pulses']])# Add edges from this new Pulse node to all the pulse Nodes linked to the specified node.
                self.WG.node[nearest_node]['pulses'].update(set([int(len(self.PG))]))# replace the entire set of pulses in "node" with only the reference to the newly formed pulse    
...
#            nearest_node = list(self.WG.neighbors(node))[j]
            ...
            j = diffs[j]
            self.PG.add_node(j,self.WG.node[node]['scale']) # Add a new node to PG with (value j,  scale equal to node's scale)           
            self.PG.add_edges_from([(i,len(self.PG)) for i in self.WG.node[node]['pulses']])# Add edges from this new Pulse node to all the pulse Nodes linked to the specified node.
            self.WG.node[nearest_node]['pulses'].update(set([int(len(self.PG))]))# replace the entire set of pulses in "node" with only the reference to the newly formed pulse    
            self.WG.nodes[nearest_node]['scale'] += self.WG.nodes[node]['scale']
            self.WG.add_edges_from([(i,nearest_node) for i in self.WG.neighbors(node) if i!=nearest_node])# Add the edges of node node to node J.
    ...
    def check_feature(self, node):
        ...
        nearest_node = [i for i in self.WG.neighbors(node) if (self.WG.nodes[i]['value']==self.WG.node[node]['value'])]
        if len(nearest_node) != 0:
            ...
            self.WG.node[nearest_node]['pulses'].update(self.WG.nodes[node]['pulses'])
            self.WG.remove_node(node)            
                ...
                if (self.WG.has_node(node)):                    
                    if self.WG.node[node]['scale']==scale_cur:# Ensure the node of still of the current scale.
                        if self.check_feature(node):#check if the node in the feature table is still a feature
                            ...
                            isPulse = False#then it is no longer a pulse and..
                        if isPulse:
                            self.add_pulse(node)
            #END FOR
    ...
    def reconstruct_full(self):
        ...
        for i in range(0, signal_len):
            ...
            self.current_signal[i] = sum([self.PG.node[x]['value'] for x in path])
        elapsed_time = time.process_time() - t
